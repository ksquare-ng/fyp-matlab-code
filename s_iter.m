...
iter_set = [1 5 10 15 20 25 50 75 100]; %Number of iterations

N_md = zeros(length(iter_set),1); 
N_fa = zeros(length(iter_set),1); 
P_md = zeros(length(iter_set),1); 
P_fa = zeros(length(iter_set),1); 

figure 

for i = 1:length(iter_set)
    iter = iter_set(i); 
...
iter=iter_set;
semilogy(iter,P_md,'ko-',iter,P_fa,'k+-');
grid on
hold
xlabel('Iteration');
ylabel('Probability');
